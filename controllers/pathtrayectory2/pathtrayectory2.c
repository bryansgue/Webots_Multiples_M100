...
static void create_trail_shape() {
  ...
  WbNodeRef existing_trail = wb_supervisor_node_get_from_def("TRAIL_2");
  if (existing_trail)
  ...
  strcat(trail_string, "DEF TRAIL_2 Shape {\n");
  ...
  strcat(trail_string, "    }\n");
  strcat(trail_string, "  }\n");
  strcat(trail_string, "  geometry DEF TRAIL_LINE_SET_2 IndexedLineSet {\n");
...
int main(int argc, char **argv) {
  ...
  WbNodeRef target_node = wb_supervisor_node_get_from_def("TARGET_2");

  // Create the TRAIL Shape which will contain the green line set.
  create_trail_shape();

  // Get interesting references to the TRAIL subnodes.
  WbNodeRef trail_line_set_node = wb_supervisor_node_get_from_def("TRAIL_LINE_SET_2");
